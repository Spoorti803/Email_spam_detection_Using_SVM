...
│   └── ...
├── train_model1.ipynb           # Trains the TF-IDF + SVM model, saves the .pkl files
├── app.ipynb                    # Flask backend (run as a notebook) — /send-email endpoint
└── README.md
```

> Note: `app.ipynb` is the Flask backend — it's written and run as a Jupyter notebook rather than a standalone `.py` script.
>
> `spam_classifier_model.pkl` and `tfidf_vectorizer.pkl` are **not included in this repo** (they're git-ignored). Run `train_model1.ipynb` first to generate them locally before running `app.ipynb` — see [Getting Started](#-getting-started) below.
...
### 3. Train the model (required first — generates the .pkl files)
```bash
pip install pandas scikit-learn nltk joblib jupyter
jupyter notebook train_model1.ipynb
```
Run all cells. This trains the TF-IDF + SVM classifier and saves `spam_classifier_model.pkl` and `tfidf_vectorizer.pkl` in the project root.

### 4. Run the backend (Flask server, run as a notebook)
```bash
pip install flask flask-cors firebase-admin joblib
```

- Download your Firebase Admin SDK service account key from the Firebase Console and save it as `firebase_credentials.json` in the project root (this file should **never** be committed — it's already in `.gitignore`).
- Make sure `spam_classifier_model.pkl` and `tfidf_vectorizer.pkl` (from step 3) are in the same folder as `app.ipynb`.

```bash
jupyter notebook app.ipynb
```
Run all cells — this starts the Flask server at `http://127.0.0.1:5002` and keeps running as long as the notebook kernel is active.
...
- **Backend runs as a Jupyter notebook** rather than a standalone script — convenient for development, but a `.py` version would be more suitable for deployment (e.g. behind gunicorn).
